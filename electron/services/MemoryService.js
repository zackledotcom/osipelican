...
class MemoryService extends events_1.EventEmitter {
    ...
    async store(chunk) {
        ...
        if (!chunk.vector) {
            chunk.vector = await this.embeddingService.getEmbedding(chunk.content);
        }
    ...
    `);
        try {
            stmt.run(chunk.id, content, JSON.stringify(chunk.metadata), chunk.vector ? new Float64Array(chunk.vector).buffer : undefined, Date.now(), Date.now(), importance, chunk.expiresAt || null, shouldCompress);
            // Update cache
    ...
    `).get(Date.now(), Date.now());
        ...
        const safeStats = (typeof stats === 'object' && stats !== null) ? stats : defaultStats;
        return {
            ...safeStats,
        };
    }
